...
class _teachersGuidePageState extends State<teachersGuidePage> {
  ...
  getImage(int index) {
    ...
    if (!imageData.containsKey(index)) {
      ...
      photosReference
          .child(getNameChild2(index))
          .getData(2 * 1024 * 1024)
          .then((data) {
        this.setState(() {
          imageFile = data!;
        });
        imageData.putIfAbsent(index,() {
      ...
      imageFile=imageData[index]!;
    }
  }

  String getNameChild1() {
    if(isEnglish){
      return "TeachersGuide";
    }else{
      return "TeachersGuideSinhala";
  ...
  String getNameChild2(int index) {
    String Final_Index = CreateFinalIndex(index);
    if(isEnglish){
      return "eGr12TG ICT-$Final_Index.jpg";
    }else{
